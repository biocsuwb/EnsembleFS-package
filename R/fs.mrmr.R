...
fs.mrmr <- function(x, y, params = list(feature.number = 100)){
  ...
  results <- mRMR.ensemble(data = feature.data, target_indices = 1,solution_count = 1, feature_count = params$feature.number)
  var.imp.new.names = apply(solutions(results)[[1]], 2, function(x, y) { return(y[x]) }, y = featureNames(feature.data))
